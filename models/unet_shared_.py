...
class SharedUNetAE(nn.Module):
    """
    One-class U-Net autoencoder shared across metals w small condition at bottleneck
    """
    def __init__(self, in_ch=3, base=64, out_ch=3, num_metals=0, emb_dim=32):
        ...
        self.conv4 = double_conv(base*12, base*4)   

        self.up3  = nn.ConvTranspose2d(base*4, base*2, 2, stride=2)
        self.conv3 = double_conv(base*6,  base*2)   

        self.up2  = nn.ConvTranspose2d(base*2, base,   2, stride=2)
        self.conv2 = double_conv(base*3,  base)     
    ...
    def apply_film(self, b, metal_ids):
        if self.num_metals == 0:
            return b
        gamma_beta = self.film(self.metal_emb(metal_ids))
        B, C, H, W = b.shape
    ...
    def forward(self, x, metal_ids=None):
        ...
        u = self.up4(b)
        u = self.conv4(torch.cat([u, x4], dim=1))
        u = self.up3(u)
        u = self.conv3(torch.cat([u, x3], dim=1))
        u = self.up2(u)
        u = self.conv2(torch.cat([u, x2], dim=1))
        u = self.up1(u)
        u = self.conv1(torch.cat([u, x1], dim=1))
        logits = self.out(u)
